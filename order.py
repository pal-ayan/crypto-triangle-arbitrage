    ...
    def update_order_status(self):
        ...
        for orderid in ls_ids:
            ...
            if os.path.exists(os.path.realpath('.') + "/csv/order_status.csv"):
                df.to_csv(os.path.realpath('.') + "/csv/order_status.csv", mode='a', header=False, index=False)
            else:
                df.to_csv(os.path.realpath('.') + "/csv/order_status.csv", index=False)
            for idx in df.index:
                ...
                if 'filled' == status:
                    if side == 'buy':
                        ...
                        self.m.execute_sql(statement.strip(), True)
                        statement = "select pair from orders where order_id = '" + order_id + "'"
                        results = self.m.execute_sql(statement.strip())
                        for result in results:
                            pair = result[0]
                        if pair is not None and profit < 0:
                            self.m.slack.post_message(pair + " sold for a loss of " + str(profit))
