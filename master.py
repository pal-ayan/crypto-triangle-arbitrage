...
class master:
    ...
    def init_markets_df(self):
        ...
        for idx in df.index:
            ...
            target_currency_short_name = df['target_currency_short_name'][idx]
            ecode = df['ecode'][idx]
            if base_currency_short_name not in base_currency_list:
                drop = True
            if base_currency_short_name in base_currency_list and target_currency_short_name in base_currency_list:
                drop = True
            if ecode != 'B':
                drop = True
            if drop:
