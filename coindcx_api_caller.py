...
class call_api():
    ...
    def get_user_balance(self, currency):
        ...
        while True:
            try:
                ...
                self._log.log_exception('error getting user balance')
                if retry_count >= 5:
                    self._log.log_error('maximum retry reached for getting user balance')
                    raise Exception('maximum retry reached for getting user balance')
                sleep(1)
                retry_count += 1
                continue
    ...
    def get_ticker(self):
        ...
        while True:
            try:
                ...
                self._log.log_exception('error generating ticker')
                if retry_count >= 5:
                    self._log.log_error('maximum retry reached for generating ticker')
                    raise Exception('maximum retry reached for generating ticker')
                sleep(1)
                retry_count += 1
                continue
    ...
    def get_latest_ticker(self):
        ...
        while run:
            if count > 5 :
                self._log.log_error('maximum retry reached for generating latest ticker')
                raise Exception('maximum retry reached for generating latest ticker')
            df = self.get_ticker()
            ticker_time = df['timestamp'].values[0]
            if (self._last_ticker_timestamp is not None) and (float(self._last_ticker_timestamp) >= float(ticker_time)):
                count += 1
                print('sleeping for 10 seconds')
                sleep(10)
                continue
            else:
                self._last_ticker_timestamp = ticker_time
                return df
    ...
    def get_trade_history(self, latest_trade_id=None):
        time_stamp = int(round(time.time() * 1000))
